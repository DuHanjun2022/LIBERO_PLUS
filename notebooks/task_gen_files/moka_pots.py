...
"""
This is the tasks to put multiple moka pots on a single stove

KITCHEN_SCENE8_put_both_moka_pots_on_the_stove

We extend the difficulty by modifying the receptacle location, and adding distractor objects

two distractors, two pots

one distractor, three pots: selecting specific moka pots:
- left and right
- middle and left

For eval: we can use subset of spatial relationship (eg. all but middle of three)
with one distractor object
- middle and right
...
train_objs = ["moka_pot", "moka_pot"]
distractor_objs = ["white_yellow_mug"]
eval_objs = ["moka_pot", "moka_pot", "moka_pot"]


pot_locs = { # divide space into 3 regions L M R
    "left": ([-0.025, 0.2], [0.175, 0.1]),
    "middle": ([-0.025, 0.0], [0.175, 0.1]),
    "right": ([-0.025, -0.2], [0.175, 0.1]),
}

stove_loc = ([-0.025, 0.], [0.175, 0.3]) # entire space
...
for background_idx, background in enumerate(backgrounds):
    ...
    stove = DestInitState(name="flat_stove")

    objs = [
        ObjectInitState(name="moka_pot", dest=stove, idx=1),
        ObjectInitState(name="moka_pot", dest=stove, idx=2)
    ]
    
    for obj in distractor_objs:
        objs.append(ObjectInitState(name=obj))

    lang = f"put both moka pots on the stove"
